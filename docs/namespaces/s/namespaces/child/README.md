[**nodejsscript**](../../../../README.md) • **Docs**

***

[nodejsscript](../../../../README.md) / [s](../../README.md) / child

# child

## Index

### Namespaces

- [exec](namespaces/exec/README.md)
- [execFile](namespaces/execFile/README.md)

### Classes

- [ChildProcess](classes/ChildProcess.md)

### Interfaces

- [ChildProcessWithoutNullStreams](interfaces/ChildProcessWithoutNullStreams.md)
- [ChildProcessByStdio](interfaces/ChildProcessByStdio.md)
- [MessageOptions](interfaces/MessageOptions.md)
- [MessagingOptions](interfaces/MessagingOptions.md)
- [ProcessEnvOptions](interfaces/ProcessEnvOptions.md)
- [CommonOptions](interfaces/CommonOptions.md)
- [CommonSpawnOptions](interfaces/CommonSpawnOptions.md)
- [SpawnOptions](interfaces/SpawnOptions.md)
- [SpawnOptionsWithoutStdio](interfaces/SpawnOptionsWithoutStdio.md)
- [SpawnOptionsWithStdioTuple](interfaces/SpawnOptionsWithStdioTuple.md)
- [ExecOptions](interfaces/ExecOptions.md)
- [ExecOptionsWithStringEncoding](interfaces/ExecOptionsWithStringEncoding.md)
- [ExecOptionsWithBufferEncoding](interfaces/ExecOptionsWithBufferEncoding.md)
- [ExecException](interfaces/ExecException.md)
- [PromiseWithChild](interfaces/PromiseWithChild.md)
- [ExecFileOptions](interfaces/ExecFileOptions.md)
- [ExecFileOptionsWithStringEncoding](interfaces/ExecFileOptionsWithStringEncoding.md)
- [ExecFileOptionsWithBufferEncoding](interfaces/ExecFileOptionsWithBufferEncoding.md)
- [ExecFileOptionsWithOtherEncoding](interfaces/ExecFileOptionsWithOtherEncoding.md)
- [ForkOptions](interfaces/ForkOptions.md)
- [SpawnSyncOptions](interfaces/SpawnSyncOptions.md)
- [SpawnSyncOptionsWithStringEncoding](interfaces/SpawnSyncOptionsWithStringEncoding.md)
- [SpawnSyncOptionsWithBufferEncoding](interfaces/SpawnSyncOptionsWithBufferEncoding.md)
- [SpawnSyncReturns](interfaces/SpawnSyncReturns.md)
- [CommonExecOptions](interfaces/CommonExecOptions.md)
- [ExecSyncOptions](interfaces/ExecSyncOptions.md)
- [ExecSyncOptionsWithStringEncoding](interfaces/ExecSyncOptionsWithStringEncoding.md)
- [ExecSyncOptionsWithBufferEncoding](interfaces/ExecSyncOptionsWithBufferEncoding.md)
- [ExecFileSyncOptions](interfaces/ExecFileSyncOptions.md)
- [ExecFileSyncOptionsWithStringEncoding](interfaces/ExecFileSyncOptionsWithStringEncoding.md)
- [ExecFileSyncOptionsWithBufferEncoding](interfaces/ExecFileSyncOptionsWithBufferEncoding.md)

### Type Aliases

- [Serializable](type-aliases/Serializable.md)
- [SendHandle](type-aliases/SendHandle.md)
- [IOType](type-aliases/IOType.md)
- [StdioOptions](type-aliases/StdioOptions.md)
- [SerializationType](type-aliases/SerializationType.md)
- [StdioNull](type-aliases/StdioNull.md)
- [StdioPipeNamed](type-aliases/StdioPipeNamed.md)
- [StdioPipe](type-aliases/StdioPipe.md)
- [ExecFileException](type-aliases/ExecFileException.md)

### Functions

- [spawn](functions/spawn.md)
- [exec](functions/exec.md)
- [execFile](functions/execFile.md)
- [fork](functions/fork.md)
- [spawnSync](functions/spawnSync.md)
- [execSync](functions/execSync.md)
- [execFileSync](functions/execFileSync.md)
